...
In addition to the lightweight :class:`DataPoint`, this module exposes a
richer :class:`DataCell` that carries full database-table metadata,
business-domain labels, lifecycle stage, provenance tracking, and arbitrary
tags.  :class:`DataCell` is used by the advanced seebook-derived modules such
as the dynamic classifier, quality engine, lineage tracker, and optimizer.
"""
...
if TYPE_CHECKING:
    from .lineage import Provenance
...
# ---------------------------------------------------------------------------
# Rich DataCell – seebook-compatible extended record
# ---------------------------------------------------------------------------

@dataclass
class DataCell:
    """Extended single record that carries full database-table metadata.

    :class:`DataCell` is a superset of :class:`DataPoint` designed for the
    advanced seebook-derived modules (dynamic classifier, quality engine,
    lineage tracker, matrix optimizer).  Compared to :class:`DataPoint` it
    adds:

    * ``table_name`` / ``schema_name`` – database source identification.
    * ``column_count``, ``row_count``, ``size_bytes`` – table statistics.
    * ``business_domain`` / ``lifecycle_stage`` – classification labels.
    * ``tags`` – arbitrary keyword tags.
    * ``provenance`` – optional :class:`~.lineage.Provenance` object that
      records where this cell's data came from and how it was derived.

    Attributes:
        t: Time coordinate (datetime).
        x: Business-phase coordinate (0–255).
        y: Normalised quantity / size metric (float).
        z: Topic / domain coordinate (non-negative integer).
        payload: Arbitrary JSON-serialisable metadata (column definitions,
            indexes, custom attributes, …).
        table_name: Source database table name.
        schema_name: Source database schema / namespace.
        column_count: Number of columns in the source table.
        row_count: Number of rows in the source table.
        size_bytes: On-disk size of the source table in bytes.
        last_updated: When the source table was last modified (if known).
        tags: Free-form keyword tags for searching and grouping.
        business_domain: Human-readable domain label used for the z-axis.
        lifecycle_stage: One of ``"new"``, ``"growth"``, ``"mature"``,
            ``"legacy"``.
        provenance: Optional lineage/provenance metadata.
    """
    ...
    business_domain: str = ""
    lifecycle_stage: str = ""
    provenance: Optional[Any] = None  # type: Optional[Provenance]
    ...
    def to_data_point(self) -> DataPoint:
        """Convert to a lightweight :class:`DataPoint` (drops rich metadata)."""
        return DataPoint(t=self.t, x=self.x, y=self.y, z=self.z, payload=self.payload)
